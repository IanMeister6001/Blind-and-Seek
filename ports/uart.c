...
16-07-2017 Toevoeging van USART3 in de UART lib
*/
...
void UARTGPS_init(void)
{
  ...
  /* --------------------------- System Clocks Configuration -----------------*/
  /* USART6 clock enable */
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART6, ENABLE); //Gebruik USART6 voor de UART GPS.
  ...
  /* Connect USART pins to AF */
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource6, GPIO_AF_USART6);   // USARTGPS_TX
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource7, GPIO_AF_USART6);  // USARTGPS_RX
...
{
    char uart_char = -1;
    if (USART_GetFlagStatus(USART6, USART_FLAG_RXNE)== SET)  // check for data available
    uart_char= USART6->DR & 0xFF; // and read the data from peripheral
    return uart_char;
}
...
if (echo)              // if output-flag set
UART3_putchar(*s);  // to read what u entered
