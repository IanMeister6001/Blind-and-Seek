...
void MboxPendTask (void *pdata)
{
	...
	while (TRUE)
	{
		...
		case '4': Uart_debug_out ^= QUEUE_DEBUG_OUT; // toggle output on/off
				  ...
				  break;
		case '5': Uart_debug_out ^= INTERRUPT_DEBUG_OUT; // toggle output on/off
		  	  	  UART_puts("\n\rinterrupt output = "); UART_puts(Uart_debug_out & INTERRUPT_DEBUG_OUT ? "ON\n\r" : "OFF\n\r");
				  break;
		case '6': Uart_debug_out ^= STUDENTSTUFF_DEBUG_OUT; // toggle output on/off
		  	  	  UART_puts("\n\rstudentsstuff output = "); UART_puts(Uart_debug_out & STUDENTSTUFF_DEBUG_OUT ? "ON\n\r" : "OFF\n\r");
				  break;
		case 's':
		case 'S': displayAllStackData(); break;                // check stack data of tasks
		}
